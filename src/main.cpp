...
int main()
{
    ...
    std::string cpp_console_output = "Hello from CPP";
    auto render_cpp_console_output = [&cpp_console_output]()
    {
        return hbox({text("CPP console output: "), text(cpp_console_output)});
    };
    auto cpp_renderer = Renderer([render_cpp_console_output]
                                 { return vbox({render_cpp_console_output()}) | frame; });

    auto cpp_container = Container::Horizontal({cpp_renderer});

    if (config->is_any_tool_from_language_loaded<CPP>())
    {
        tab_entries.push_back("CPP");
        tab_content->Add(cpp_container);
    };
